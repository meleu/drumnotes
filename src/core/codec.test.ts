...
/** A serialised store holding whatever is offered as its current pattern, and
 *  whatever is offered as its library. */
function storeWith(current: unknown, library: Record<string, unknown> = {}): string {
...
describe('parseStore', () => {
  ...
  const unreadableCurrent: [name: string, stored: unknown][] = [
  ...
  const unreadable: [name: string, stored: string | null][] = [
    ...unreadableStores,
    ...unreadableCurrent.map<[string, string]>(([name, current]) => [name, storeWith(current)]),
  ];
